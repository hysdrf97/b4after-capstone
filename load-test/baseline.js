...
export function handleSummary(data) {
  ...
  const report = {
    ...
    phase: "FASE 1 — Baseline (No Cache, No Queue)",

    // ── Traffic Distribution ──
    traffic_distribution: {
      "GET /users/:id       (read-heavy)": {
        requests: getUserReqs,
        percentage: pct(getUserReqs),
      },
      "GET /transactions/:id (read-heavy)": {
        requests: getTxReqs,
        percentage: pct(getTxReqs),
      },
      "POST /transactions    (write-heavy)": {
        requests: createTxReqs,
        percentage: pct(createTxReqs),
      },
    },
  ...
  // Cetak ke terminal juga
  console.log('\n========== BASELINE REPORT ==========');
  console.log(json);
  console.log('=====================================\n');

  return {
    'results/baseline_report.json': json,
    stdout: '\n✅ Report saved to results/baseline_report.json\n',
  };
}